...
process	main(void)
{
	...
	*/
	...
	// Problem 4.3
	/*
	int cpuUsageTest1PID1 = create(cpubnd, 512, 1, "Test 1", 0);
	int cpuUsageTest1PID2 = create(cpubnd, 512, 1, "Test 2", 0);
	int cpuUsageTest1PID3 = create(iobnd, 512, 1, "Test 3", 0);
	int cpuUsageTest1PID4 = create(iobnd, 512, 1, "Test 4", 0);

	struct procent *prptr1 = &proctab[cpuUsageTest1PID1];
	struct procent *prptr2 = &proctab[cpuUsageTest1PID2];
	struct procent *prptr3 = &proctab[cpuUsageTest1PID3];
	struct procent *prptr4 = &proctab[cpuUsageTest1PID4];

	kprintf("Test 1 Before CPU Usage: %u\n", prptr1->prcpuused);
	kprintf("Test 2 Before CPU Usage: %u\n", prptr2->prcpuused);
	kprintf("Test 3 Before CPU Usage: %u\n", prptr3->prcpuused);
	kprintf("Test 4 Before CPU Usage: %u\n", prptr4->prcpuused);

	resume(cpuUsageTest1PID1);
	resume(cpuUsageTest1PID2);
	resume(cpuUsageTest1PID3);
	resume(cpuUsageTest1PID4);
	*/

	// Problem 5
	int attackerPID = create(stacksmashA, 1024, 1, "stacksmashA", 0);
	int victimPID = create(stacksmashV, 1024, 1, "stacksmashV", 0);
	resume(victimPID);
	resume(attackerPID);
